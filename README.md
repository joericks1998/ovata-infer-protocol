...
codec spoken between a client and the inference daemon.

## Why it lives on its own

Two codebases speak this protocol — the daemon that serves it, and the Jade
...
the ceilings bounding a runaway daemon existed on only one side. Those are
...
repository visibility: the language repo is public, the daemon repo is not, and
...
- **Inference daemon** — consumes this as a git submodule and a workspace member.
...
| File | Contents |
|---|---|
| `request.rs` | `InferenceRequest` — client → daemon, length-prefixed JSON |
| `response.rs` | `Frame` — daemon → client, `[type][len][payload]`, plus the `tag` constants |
